/**
 * @file 04岩崎悠紀DSP2-2-4.c
 * @brief DSP2-2-4・YCC変換を実現する
 * @authur 岩崎悠紀
 * @date 2019/06/06
 */
#include <stdio.h>
...
void get_bmp_data(FILE *fp, bmp *bmp_data, int32_t width, int32_t height);
void malloc_bmp_rgb_data(bmp *bmp_data, int32_t width, int32_t height);
void write_ycc_y_data(char *filename, bmp *bmp_data);
...
int main(){
    ...
    printf("==============================\n");
    printf("=============YCC==============\n");
    printf("1個目：    Y   (%3lf, %3lf, %3lf)\n", bmp_data->ycc_data[0][0].y, bmp_data->ycc_data[0][0].cb, bmp_data->ycc_data[0][0].cr);
    printf("5000個目： CB  (%3lf, %3lf, %3lf)\n", bmp_data->ycc_data[41][79].y, bmp_data->ycc_data[41][79].cb, bmp_data->ycc_data[41][79].cr);
    printf("14400個目：CR  (%3lf, %3lf, %3lf)\n", bmp_data->ycc_data[119][119].y, bmp_data->ycc_data[119][119].cb, bmp_data->ycc_data[119][119].cr);
    printf("==============================\n");
    write_ycc_y_data("data/sample.csv", bmp_data);
}
...
void get_bmp_data(FILE *fp, bmp *bmp_data, int32_t width, int32_t height){
    int w, h;
    rgb **rgb_data = bmp_data->rgb_data;
    ycc **ycc_data = bmp_data->ycc_data;
    fseek(fp, BMP_RGB_DATA_BEGIN, SEEK_SET);
    for(h=0; h<height; h++){
        for(w=0; w<width; w++){
            fread(&(rgb_data[h][w].blue), 1, 1, fp);
            fread(&(rgb_data[h][w].green), 1, 1, fp);
            fread(&(rgb_data[h][w].red), 1, 1, fp);
            ycc_data[h][w].y = (double)rgb_data[h][w].blue*0.114 \
                + (double)rgb_data[h][w].green*0.587 \
                + (double)rgb_data[h][w].red*0.299;
            ycc_data[h][w].cb = 0.564 * ((double)rgb_data[h][w].blue - ycc_data[h][w].y);
            ycc_data[h][w].cr = 0.713 * ((double)rgb_data[h][w].red - ycc_data[h][w].y);
        }
    }
}
...
void malloc_bmp_rgb_data(bmp *bmp_data, int32_t width, int32_t height){
    int i;
    bmp_data->rgb_data = (rgb **)malloc(sizeof(rgb *)*height);
    bmp_data->ycc_data = (ycc **)malloc(sizeof(ycc *)*height);
    for(int i=0; i<width; i++){
        bmp_data->rgb_data[i] = (rgb *)malloc(sizeof(rgb)*width);
        bmp_data->ycc_data[i] = (ycc *)malloc(sizeof(ycc)*width);
    }
}

/**
 * @brief BMPデータのYCCのY成分をファイル出力する関数。
...
void write_ycc_y_data(char *filename, bmp *bmp_data){
    ...
    for(i=0; i<bmp_data->info_header.height; i++){
        for(j=0; j<bmp_data->info_header.width; j++){
            fprintf(fp, "%lf, ", bmp_data->ycc_data[i][j].y);
        }
        fprintf(fp, "\n");
    }
}