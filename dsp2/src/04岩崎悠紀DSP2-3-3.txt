/**
 * @file 04岩崎悠紀DSP2-3-3.c
 * @brief DSP2-3-3・固有値と固有ベクトルを複数組求める
 * @authur 岩崎悠紀
 * @date 2019/07/10
 */
#include <stdio.h>
...
Matrix coe_matrix(Matrix mat, double c);
...
int main(){
    ...
    malloc_matrix(&v_next_t, 1, NUM);
    malloc_matrix(&lambda, 1, 1);
    malloc_matrix(&sub, NUM, 1);
    malloc_matrix(&buf1, NUM, NUM);
    malloc_matrix(&buf2, NUM, NUM);
    do{
        do {
        ...
        printf("固有値\n");
        print_matrix(lambda);
        transpose_matrix(v_next, &v_next_t);
        multi_matrix(v_next, v_next_t, &buf1);
        buf1 = coe_matrix(buf1, lambda.data[0][0]);
        sub_matrix(Ann, buf1, &buf2);
        copy_matrix(buf2, &Ann);
        i++;
    } while(i<3);
...
/**
 * @brief 与えられた行列に係数cをかける関数。
 * @param mat 対象のMatrix。
 * @param c 係数。
 * @return matとcの乗算の演算結果。
 */
Matrix coe_matrix(Matrix mat, double c){
    int i, j, n;
